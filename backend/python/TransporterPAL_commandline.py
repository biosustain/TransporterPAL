...
contact="cfb-sa-ncbi-api@biosustain.dtu.dk"
headers ={'User-Agent':'Python {}'.format(contact)}

with open(r"python/species.v11.5.txt", newline='') as f:
...
print("Step 1 - Done - " + str(time.time() - step1_time));
# Sync call 1 end
...
print("Step 2 - Done - " + str(time.time() - step2_time));
# Sync call 2 end
...
print("Step 3 - Done - " + str(time.time() - step3_time));
# Async call 3 end

#    print(len(uniprot_entries))   
...
print("Step 4 - Done - " + str(time.time() - step4_time));
# Sync call 4 end
...
#    print("Number of entries processed is",start)
    try:
...
#            TCDB=json.loads(response_uniprot.text)
          
            TCDB.append(json.loads(response_uniprot.text))

    except:
        sys.exit("Connection to host rest.uniprot.org/idmapping failed")
#        print("Sorry, I can't reach Uniprot at the moment, try again later")
#print(TCDB)
...
print("Step 5 - Done - " + str(time.time() - step5_time));
# Sync call 5 end
...
with open("python/python_files/" + file_names + '.fasta','w') as f:
     f.write(''.join(fasta))


header=['Entry name', 'Protein name', 'Organism', 'TCDB', 'Scores']
with open ("python/python_files/" + file_names + '.txt', "w", newline='') as f:
